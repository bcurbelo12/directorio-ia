...
function createCard(company) {
  ...
  if (rgb) {
    card.style.setProperty('--card-color', company.color);
    card.style.setProperty('--card-color-rgb', `${rgb.r}, ${rgb.g}, ${rgb.b}`);
  }

  const isFavorite = favoriteCompanies.has(company.id);

  card.innerHTML = `
    <div class="card-header">
      <div class="card-icon">${getInitials(company.aiName)}</div>
      <div class="card-title-group">
        <h3>${company.aiName}</h3>
        <p class="card-company">${company.companyName}</p>
      </div>
      <button class="card-favorite ${isFavorite ? 'active' : ''}" data-id="${company.id}" title="${isFavorite ? 'Eliminar de favoritos' : 'Añadir a favoritos'}">
        ★
      </button>
    </div>
    <p class="card-description">${company.description}</p>
    <div class="card-footer">
      <div class="card-category">${categoryTranslations[company.category] || company.category}</div>
      <div class="card-location">${company.location.split(',')[0]}</div>
    </div>
  `;
...
function toggleFavorite(companyId) {
  if (favoriteCompanies.has(companyId)) {
    favoriteCompanies.delete(companyId);
  } else {
    favoriteCompanies.add(companyId);
  }
  
  // Update favorite button
  const favoriteBtn = document.querySelector(`[data-id="${companyId}"]`);
  if (favoriteBtn) {
    const isActive = favoriteCompanies.has(companyId);
    favoriteBtn.classList.toggle('active', isActive);
    favoriteBtn.title = isActive ? 'Eliminar de favoritos' : 'Añadir a favoritos';
  }
...
function addToRecentlyViewed(company) {
  recentlyViewed = recentlyViewed.filter(c => c.id !== company.id);
  recentlyViewed.unshift(company);
...
function updateFavoritesCount() {
  ...
  if (favoritesCountElement) {
    favoritesCountElement.textContent = favoriteCompanies.size;
  }
}

// Render cards with virtual scrolling optimization for better performance
function renderCards(companies) {
  ...
  if (totalCountElement) {
    totalCountElement.textContent = companies.length;
  }
  
  companies.forEach((company, index) => {
    const card = createCard(company);
    card.style.animationDelay = `${(index % 20) * 0.05 + 0.1}s`;
    cardsGrid.appendChild(card);
  });
}

// Enhanced filter function with performance optimizations
function filterCompanies() {
  ...
  if (currentFilter !== 'all') {
    if (currentFilter === 'favorites') {
      ...
      filteredCompanies = filteredCompanies.filter(company => 
        company.category === currentFilter
      );
    }
  }
  ...
  if (currentSearchTerm) {
    ...
    filteredCompanies = filteredCompanies.filter(company =>
      company.aiName.toLowerCase().includes(searchTerm) ||
      company.companyName.toLowerCase().includes(searchTerm) ||
      company.description.toLowerCase().includes(searchTerm) ||
      company.location.toLowerCase().includes(searchTerm) ||
      company.founders.some(founder => founder.toLowerCase().includes(searchTerm))
    );
  }
...
function openModal(company) {
  ...
  const modalAiName = document.getElementById('modalAiName');
  const modalCompanyName = document.getElementById('modalCompanyName');
  const modalDescription = document.getElementById('modalDescription');
  const modalFounders = document.getElementById('modalFounders');
  const modalFoundingDate = document.getElementById('modalFoundingDate');
  ...
  const rgb = hexToRgb(company.color);
  if (rgb) {
    document.documentElement.style.setProperty('--modal-color', company.color);
  }

  // Populate modal content
  modalIcon.textContent = getInitials(company.aiName);
  modalIcon.style.background = company.color;
  modalAiName.textContent = company.aiName;
  modalCompanyName.textContent = company.companyName;
  modalDescription.textContent = company.description;
  modalFounders.textContent = company.founders.join(', ');
  modalFoundingDate.textContent = company.foundingDate;
  modalLocation.textContent = company.location;
  modalFact.textContent = company.relevantFact;
...
// Random company feature
function showRandomCompany() {
  const randomIndex = Math.floor(Math.random() * companiesData.companies.length);
  const randomCompany = companiesData.companies[randomIndex];
  openModal(randomCompany);
  addToRecentlyViewed(randomCompany);
}

// Event listeners
document.addEventListener('DOMContentLoaded', () => {
  // Initial render
  renderCards(companiesData.companies);
  
  // Update stats
  if (totalCountElement) {
    totalCountElement.textContent = companiesData.companies.length;
  }
  ...
  if (randomBtn) {
    randomBtn.addEventListener('click', showRandomCompany);
  }
...
function exportCompanies(format = 'json') {
  ...
  if (format === 'json') {
    ...
    link.href = url;
    link.download = 'ai-companies.json';
    link.click();
...
// Initialize previous width for responsive handling
window.previousWidth = window.innerWidth;

console.log(`🚀 Directorio de IA cargado con ${companiesData.companies.length} empresas de inteligencia artificial`);